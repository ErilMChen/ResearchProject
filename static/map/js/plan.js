...
function saveBusRoute(){
    ...
    let url = 'paln/'+'?start_stop='+start+'&end_stop='+end +'&date='+date +'&time='+time +'&plan_name='+plan_name;
    fetch(url, {
        method:'GET'}).then(function(response) {
            // read data from django server and prase to json
            console.log("add plan ok")
...
function loadPlanFromJson(){
    ...
    for(var i=localStorage.length - 1 ; i >=0; i--){
        ...
        writeLine("Plan Name: " + locations.planName, container)
